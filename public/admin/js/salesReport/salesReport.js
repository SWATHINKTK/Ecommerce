...
const salesDateFilter = document.getElementById('salesFilter');

if (salesDateFilter) {

	salesDateFilter.addEventListener('click', (event) => {
		...
		const validate = document.getElementById('saleDateValidate');
		validate.innerHTML = '';

		if(startDate.trim() == '' || endDate.trim() == ''){
			validate.innerHTML = '* enter two dates';
		}else if(startDate.length != 10 || endDate.length != 10){
			validate.innerHTML = '* enter proper date';
		}else if(startDate > endDate){
			alert(startDate.length)
			validate.innerHTML = '* set the start date is less than end date';
		}else{
			window.location.href = `/admin/saleReportFilter?startDate=${startDate}&endDate=${endDate}`
		}
	})
}
